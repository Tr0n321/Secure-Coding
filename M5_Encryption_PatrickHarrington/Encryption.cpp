...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
  ...
  for (size_t i = 0; i < source_length; ++i)
  {
    ...
    // The modulus operator wraps the key index when the data is longer than the key.
...
{
  // Open the input file in binary mode so that all characters are preserved exactly.
  ...
  // Read the entire file into a string using a string stream buffer.
...
{
  // Open the output file in binary mode so encrypted characters are not altered.
...
int main()
{
  ...
  // Stop the program if the source file was not loaded successfully.
  if (source_string.empty())
  {
    std::cerr << "No source data was loaded. Program ending." << std::endl;
    return 1;
  }
